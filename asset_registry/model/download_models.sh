...
download_hf_repo() {
    ...
    local local_dir="$2"
    mkdir -p "$local_dir"
    echo "[models] Downloading $repo_id ..."
...
if [ ! -f "$HAMER_ENCODER_CKPT" ]; then
    ...
    # Resume partial download if it exists
    for part in "$HAMER_ARCHIVE".*.part; do
        if [ -f "$part" ]; then
            echo "[models] Found partial download, resuming..."
            mv "$part" "$HAMER_ARCHIVE"
            break
        fi
    done
